...
void buildStage(String DOCKER_OS, String STAGE_PARAM) {
    sh """
        set -o xtrace
        mkdir test
        wget \$(echo ${GIT_REPO} | sed -re 's|github.com|raw.githubusercontent.com|; s|\\.git\$||')/${GIT_BRANCH}/hll/hll_builder.sh -O builder.sh
...
pipeline {
    ...
    parameters {
        ...
        string(
            ...
            description: 'URL for hll packaging repository',
            name: 'GIT_REPO')
        string(
            defaultValue: '2.19',
            description: 'Tag/Branch for pg_hll repository',
            name: 'PG_BRANCH')
        string(
            defaultValue: '16.1',
            description: 'Tag/Branch for pg_hll packaging repository',
            name: 'GIT_BRANCH')
        string(
            defaultValue: '1',
    ...
    stages {
        stage('Create pg_hll source tarball') {
            steps {
                slackNotify("#releases-ci", "#00FF00", "[${JOB_NAME}]: starting build for ${GIT_BRANCH} - [${BUILD_URL}]")
                cleanUpWS()
                buildStage("oraclelinux:9", "--get_sources=1")
                sh '''
                   REPO_UPLOAD_PATH=$(grep "UPLOAD" test/percona-hll.properties | cut -d = -f 2 | sed "s:$:${BUILD_NUMBER}:")
                   AWS_STASH_PATH=$(echo ${REPO_UPLOAD_PATH} | sed  "s:UPLOAD/experimental/::")
                   echo ${REPO_UPLOAD_PATH} > uploadPath
                   echo ${AWS_STASH_PATH} > awsUploadPath
                   cat test/percona-hll.properties
                   cat uploadPath
                '''
                script {
        ...
        stage('Build pg_hll generic source packages') {
            parallel {
                stage('Build pg_hll generic source rpm') {
                    agent {
                        label params.CLOUD == 'Hetzner' ? 'docker-x64-min' : 'docker'
                    }
                    steps {
        ...
        stage('Build pg_hll RPMs/DEBs/Binary tarballs') {
            parallel {
