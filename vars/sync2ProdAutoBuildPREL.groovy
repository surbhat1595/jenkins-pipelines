                ...
                sh """
                    ...
                    ssh -o StrictHostKeyChecking=no -i ${KEY_PATH} ${USER}@repo.ci.percona.com ' \
                        ...
                        pushd ${path_to_build}/binary
                            ...
                            # For Architecture: all packages, reprepro uses a single shared
                            # pool file across codenames. Pick one canonical binary per
                            # _all.deb basename, then remove the package from ALL repo
                            # codenames so the stale pool file is fully dereferenced
                            # before we push the new binary.
                            ...
                            for all_deb_base in \$(ls \${canonical_dir} 2>/dev/null); do
                                ...
                                for push_dist in \$(ls -1 debian); do
                                    /usr/local/reprepro5/bin/reprepro -Vb /srv/repo-copy/${REPO_NAME}/apt -C ${DESTINATION} remove \${push_dist} \${pkg_name} 2>/dev/null || true
                                done
                            done
                            ...
                            for dist in `ls -1 debian`; do
                                for deb in `find debian/\${dist} -name '*.deb'`; do
                                 ...
                                 # Use the canonical file for _all.deb packages
                                 if [ -f \${canonical_dir}/\${pkg_fname} ]; then
                                     deb=\$(cat \${canonical_dir}/\${pkg_fname})
                                 fi
