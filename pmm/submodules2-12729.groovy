    ...
    withCredentials([string(credentialsId: 'GITHUB_API_TOKEN', variable: 'GITHUB_API_TOKEN')]) {
        sh """
            curl -v -X POST \
                ...
                -d "{\\"body\\":\\"${COMMENT}\\"}" \
                "https://api.github.com/repos/\$(echo ${CH_URL} | cut -d '/' -f 4-5)/issues/${CH_ID}/comments"
        """
    }
}
...
pipeline {
    ...
    stages {
        ...
        stage('Build client binary') {
            steps {
                script {
                    env.BRANCH_NAME = sh(returnStdout: true, script: "cat apiBranch").trim()
                    ...
                    withCredentials([string(credentialsId: 'GITHUB_API_TOKEN', variable: 'GITHUB_API_TOKEN')]) {
                        ...
                        sh """
                            curl -v -X POST \
                                ...
                                -H "Authorization: token ${GITHUB_API_TOKEN}" \
                                "https://api.github.com/repos/\$(echo ${CH_URL} | cut -d '/' -f 4-5)/actions/workflows/jenkins-dispatch.yml/dispatches" \
                        ...
                        sh """
                            curl -v -X POST \
                                -H "Accept: application/vnd.github.v3+json" \
                                -H "Authorization: token ${GITHUB_API_TOKEN}" \
                                "https://api.github.com/repos/\$(echo ${CH_URL} | cut -d '/' -f 4-5)/actions/workflows/pmm-cli.yml/dispatches" \
                                -d '{"ref":"${GIT_BRANCH}","inputs":{"client_tar_url":"${CLIENT_URL}","sha":"${FB_COMMIT_HASH}"}}'
                        """
                        ...
                        sh """
                            curl -v -X POST \
                                -H "Accept: application/vnd.github.v3+json" \
                                -H "Authorization: token ${GITHUB_API_TOKEN}" \
                                "https://api.github.com/repos/\$(echo ${CH_URL} | cut -d '/' -f 4-5)/actions/workflows/pmm2-testsuite.yml/dispatches" \
                                -d '{"ref":"${GIT_BRANCH}","inputs":{"server_image":"${IMAGE}","client_image":"${CLIENT_IMAGE}","sha":"${FB_COMMIT_HASH}", "pmm_qa_branch": "${PMM_QA_GIT_BRANCH}", "client_version": "${CLIENT_URL}"}}'
                        """
                        ...
                        sh """
                            curl -v -X POST \
                                -H "Accept: application/vnd.github.v3+json" \
                                -H "Authorization: token ${GITHUB_API_TOKEN}" \
                                "https://api.github.com/repos/\$(echo ${CH_URL} | cut -d '/' -f 4-5)/actions/workflows/pmm2-ui-tests-fb.yml/dispatches" \
                                -d '{"ref":"${GIT_BRANCH}","inputs":{"server_image":"${IMAGE}","client_image":"${CLIENT_IMAGE}","sha":"${FB_COMMIT_HASH}", "pmm_qa_branch": "${PMM_QA_GIT_BRANCH}", "pmm_ui_branch": "${PMM_UI_TESTS_GIT_BRANCH}", "client_version": "${CLIENT_URL}"}}'
                        """
                        ...
                        sh """
                            curl -v -X POST \
                                -H "Accept: application/vnd.github.v3+json" \
                                -H "Authorization: token ${GITHUB_API_TOKEN}" \
                                "https://api.github.com/repos/\$(echo ${CH_URL} | cut -d '/' -f 4-5)/actions/workflows/trivy_scan.yml/dispatches" \
                                -d '{"ref":"${GIT_BRANCH}","inputs":{"server_image":"${IMAGE}","client_image":"${CLIENT_IMAGE}","sha":"${FB_COMMIT_HASH}"}}'
                        """
                    }
                }
