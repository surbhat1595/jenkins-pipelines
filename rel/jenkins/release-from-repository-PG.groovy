                        ...
                        if [ ${SKIP_REPO_SYNC} = false ]; then
                            ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -i ${KEY_PATH} ${USER}@repo.ci.percona.com << 'ENDSSH'
                                ...
                                if [[ ${REVERSE} = true ]]; then
                                    ...
                                    rsync \${RSYNC_TRANSFER_OPTS} 10.30.9.32:/www/repo.percona.com/htdocs/\${MAJOR_REPO}/* /srv/repo-copy/\${MAJOR_REPO}/
                                else
...
ENDSSH
    ...
    post {
        always {
            script {
                currentBuild.description = "Repo: ${REPOSITORY}/${COMPONENT}"
            }
        }
    }
}
